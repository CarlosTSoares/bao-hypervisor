...
{
    // By checking platform.arch.smmu.base we verify if a smmu is present in the platform
    if (cpu()->id == CPU_MASTER && platform.arch.smmu.base) {
        console_printk("[BAO-IOMMU] Platform has IOMMU\n");
        smmuv3_init();
        return true;
    }
